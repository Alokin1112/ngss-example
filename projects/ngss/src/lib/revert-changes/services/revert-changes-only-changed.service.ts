...
import { getDiff, rdiffResult, applyDiff } from 'recursive-diff';
...
export class RevertChangesOnlyChangedService<T> implements RevertChangesService<T> {
  ...
  */
  ...
  saveChanges<A>(newState: T, handledAction: ActionInterface<A>): void {
    ...
    const diff = getDiff(previousUpToDateState, newState);
    const diffState: RevertChangesSavedState<rdiffResult[]> = { data: diff, dateTime: new Date(), actionType: handledAction.getType() };
  ...
  private shiftArrayIfNeeded() {
    while (this.stateService.getLength() > this.options.maxPreviousStates + 2) {
      ...
      const newInitialState = {
        ...(savedStates[1] || {}),
        data: applyDiff(savedStates[0]?.data as T, savedStates[1]?.data as rdiffResult[])
